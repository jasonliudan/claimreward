...
export function StakingPage() {
  ...
  const lpPoolApy = useLpStakingPoolApy();
  const lpMaturity = useLpStakingMaturity();
  const lpTvl = useLpStakingTVL();
  const lpPoolOpen = useLpStakingPoolOpen();
  const lpStake = useLpStakeAmount();
  const lpAccumulatedRewards = useLpStakingAccumulatedRewards();
  const lpClaimableRewards = useLpStakingRewardsWithdrawable()
    ? lpAccumulatedRewards
    : 0;
  const lpSTakeTime = useLpStakingTime();
